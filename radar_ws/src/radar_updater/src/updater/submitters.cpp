 ...
 * @return serial_data 串口数据
 */
void Updater::enemy_position_data_pack_to_sentry(uint8_t* serial_data, std::map<int, info::enemy_robot_position> enemy_robot_positions, int req)
{
    // 帧头
    uint8_t frame_header[5]
        = { 0xA5, 0x0A, 0, (uint8_t)req }; // SOF, 数据长度高8位, 数据长度低8位, 包序号
    ...
    for (int i = 0; i < 6; i++) {
        info::robot_position_to_sentry single_enemy_position;
        ...
        reinterpret_cast<info::robot_position_to_sentry&>(serial_data[13 + (i * sizeof(single_enemy_position))]) = single_enemy_position;
    }
}
}